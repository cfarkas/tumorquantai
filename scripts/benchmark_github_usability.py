...
def render_markdown(snapshot: Mapping[str, Any], snapshot_path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "- **Network, GPU, or gated weights in the first success path:** those dependencies make normal CI and setup diagnosis fragile, so the first result must be structural and offline-capable.",
            "- **A magic wrapper that hides execution details:** a concise façade is useful only if it preserves direct Nextflow/run-script access, prints the expanded command with secrets redacted, and records provenance.",
